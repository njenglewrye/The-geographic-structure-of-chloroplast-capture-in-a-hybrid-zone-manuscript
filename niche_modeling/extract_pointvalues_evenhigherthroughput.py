...
# This code only applies if only individual missing data should be removed, but the overall pixel keps
	# Delete any -9999 at this point -- will break positional associations among variable PNO files
		...
		# Write the layer
		try:
			os.mkdir("./pnos_directsampling_no_missing_data_no_point_associations")
		except: 
			pass
		with open("./pnos_directsampling_no_missing_data_no_point_associations/{0}_pno_{1}.csv".format(variable, species), 'w+') as writefile:
