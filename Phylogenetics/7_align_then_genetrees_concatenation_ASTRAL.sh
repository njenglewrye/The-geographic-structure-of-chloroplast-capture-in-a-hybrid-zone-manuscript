...
#########
# Run concatenation in RAxML
#########

# Concatenate alignments

pxcat -s *.90missing.aligned.fa -p partition.90missing.txt -o concatenated.90missing.nickdissertation.fasta
...
python3 removemissingdatataxa.py concatenated.90missing.nickdissertation.fasta concatenated.90missing.50taxadeleted.nickdissertation.fasta 0.5


# Run concatenated tree

raxml-ng -all --model partition.90missing.txt --msa concatenated.90missing.50taxadeleted.nickdissertation.fasta --prefix nickdissertation --seed $RANDOM --tree pars{10} --threads 40
...
cd Astral
# Run ASTRAL
java -jar astral.5.7.8.jar -i all_gene_trees.bs10.tre -o astral.nickfinal.tre 2> astral.nickfinal.tre.log
